...
/**
 * Компонент поля типа float (число с плавающей точкой).
 * Поддерживает cardinality: one и many через Form.List.
 */
export const PathFloatField: React.FC<PropsPathFieldBase> = ({ path, fieldNamePrefix, readonly }) => {
  ...
  if (path.cardinality === 'many') {
    return (
      <PathListField
        ...
        disabled={disabled}
        renderField={() => (
          <InputNumber style={{ width: '100%' }} step={0.01} placeholder={placeholder} disabled={disabled} />
        )}
        renderAddField={add => (
          <InputNumber
            style={{ width: '100%' }}
